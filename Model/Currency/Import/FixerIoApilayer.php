 ...
 */
namespace SchneidewindIt\FixerIoApilayer\Model\Currency\Import;

use Magento\Store\Model\ScopeInterface;

class FixerIoApilayer extends \Magento\Directory\Model\Currency\Import\AbstractImport
{

    /**
     * @var string
     */
    const CURRENCY_CONVERTER_URL = 'https://api.apilayer.com/fixer/latest?base={{CURRENCY_FROM}}&symbols={{CURRENCY_TO}}';
    ...
    private function convertBatch(array $data, string $currencyFrom, array $currenciesTo): array
    {
        ...
        if (empty($accessKey)) {
            $this->_messages[] = __('No API Key was specified or an invalid API Key was specified.a');
        ...
        set_time_limit(0);
        try {
            $response = $this->getServiceResponse($url, 0, $accessKey);
        } finally {
     ...
     * @return array
     */
    private function getServiceResponse(string $url, int $retry = 0, string $accessKey = ''): array
    {
    ...
    private function validateResponse(array $response, string $baseCurrency): bool
    {
        ...
        $errorCodes = [
            101 => __('No API Key was specified or an invalid API Key was specified.c'),
